...
internal struct C2S2
{
    ...
    /// </summary>
    public HandshakeRandomBytes RandomEcho;

    [UnscopedRef]
    public Span<byte> RandomEchoSpan => RandomEcho;

    [UnscopedRef]
    public readonly ReadOnlySpan<byte> RandomEchoReadOnlySpan => RandomEcho;

    public C2S2(in C1S1 recvPeerMessage, uint time)
    {
        Timestamp = recvPeerMessage.Timestamp;
        Timestamp2 = BigEndianUInt32.FromHost(time);
        recvPeerMessage.RandomReadOnlySpan.CopyTo(RandomEchoSpan);
    }
}
