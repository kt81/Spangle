...
internal sealed class HLSPlaylist(
    string directory,
    string? mapUri = null,
    double? partTargetDuration = null,
    Action<string, long, int>? onUpdated = null)
{
    ...
    private void Write(bool ended)
    {
        ...
        if (partTargetDuration is { } partTarget)
        {
        ...
        {
            (string name, double duration) = _window[i];
            if (partTargetDuration is not null && i == _window.Count - 1)
            {
        ...
        if (partTargetDuration is not null)
        {
        ...
        onUpdated?.Invoke(text, CurrentMsn, _currentParts.Count - 1);
    }
