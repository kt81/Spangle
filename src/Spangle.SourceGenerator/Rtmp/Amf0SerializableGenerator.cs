...
""");
    ...
    private static void Emit(SourceProductionContext context, GeneratorAttributeSyntaxContext source)
    {
        ...
        var publicMembers = typeSymbol.GetMembers()
            ...
            .OrderBy(x =>
                (int)x.GetAttributes().First(attr => attr.AttributeClass?.Name is FieldAttributeName).ConstructorArguments
                    .First().Value!);

        IList<string> serializerMethods = new List<string>();
        foreach (ISymbol fieldOrProp in publicMembers)
        {
            string? serializerMethod = GetFieldSerializerMethodString(fieldOrProp);
            if (serializerMethod is null)
            {
                context.ReportDiagnostic(Diagnostic.Create(DiagnosticDescriptors.NotSupportedTypeField,
                    typeNode.Identifier.GetLocation(), GetFieldName(fieldOrProp)));
                return;
            }

            serializerMethods.Add(serializerMethod);
        }
...
{{accessibility}} partial struct {{typeSymbol.Name}}
{
    ...
    /// </summary>
    public int {{MethodNameToAdd}}(PipeWriter writer)
    {
        int total = 0;
""");
        foreach (string method in serializerMethods)
        {
...
""");
    ...
    private static string GetFieldName(ISymbol fieldOrProp)
    {
    ...
    private static string? GetFieldSerializerMethodString(ISymbol fieldOrProp)
    {
        string typeName = GetFieldName(fieldOrProp);
